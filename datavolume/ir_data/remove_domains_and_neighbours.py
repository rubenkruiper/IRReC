...
def remove_neighbours_from_file(doc_fp: Path):
        """ 
        Open a document, remove the 'fifiltered_NER_labels_domains' and 'neighbours', write back to file.
        """
        document = CustomDocument.load_document(doc_fp)
        updated_contents = []
        for c in document.all_contents:
            c.filtered_NER_labels_domains = []
            c.neighbours = []
            updated_contents.append(c)
        document.replace_contents(updated_contents)
        document.write_document()

# ------ script
input_path = Path("foreground_pdf_converted/")
json_filepaths = [x for x in input_path.glob("*.json")]
[remove_neighbours_from_file(fp) for fp in json_filepaths]
print("removed all neighbours and domain labels")
